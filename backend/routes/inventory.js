...
router.put('/:id/backpack', async (req, res) => {
    ...
    if(req.body._id == null) 
        return res.status(400).send('Bad request: none item._id value in request body.');

    const { error } = validateItemID(req.body);
    if(error) return res.status(400).send(error.details[0].message);

    const item = await Item.findById(req.body._id)
        .catch( err => { 
            console.error(`Bad request. The given ID: ${req.body._id} was not valid. ${err}`); 
            return null; 
        });
    if(!item) return res.status(400).send(`Item with given id ${req.body._id} was not found`);
